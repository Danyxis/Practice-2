...
namespace Task_4
{
    internal class Program
    {
        ...
        {
            int M_matrix = 7, N_matrix = 5; // M, i - строка; N,j - столбец
            ...
            while (currentNumber <= M_matrix * N_matrix) // Пока все ячейки матрицы не будут заполнены
            {
                ...
                if (top <= bottom) // Если верхняя граница меньше нижней или они встретились, то заполняем только справа налево
                {
                ...
                if (left <= right) // Если левая граница меньше правой или они встретились, то заполняем матрицу только снизу вверх
                {
